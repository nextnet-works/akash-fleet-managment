...
const deployAllBiddersSDL = async (respondersLength) => {
    ...
    const deploymentRes = await (0, createDeployment_1.createDeployment)(consts_1.RAW_SDL_T2);
    await new Promise((resolve) => setTimeout(resolve, 15000)); // Wait 15 seconds
    const bids = await (0, bids_1.fetchBids)(deploymentRes.id.dseq, deploymentRes.id.owner);
    if (bids.length === 0) {
