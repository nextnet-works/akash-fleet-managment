...
async function loadPrerequisites(sdlPath = consts_1.RAW_SDL_T1) {
  const wallet = await cosmwasm_1.DirectSecp256k1HdWallet.fromMnemonic(
    process.env.AKASH_MNEMONIC,
    {
      prefix: "akash",
    },
  );
  const myRegistry = new cosmwasm_1.Registry(
    (0, stargate_2.getAkashTypeRegistry)(),
  );
  const client = await stargate_1.SigningStargateClient.connectWithSigner(
    consts_1.RPC_ENDPOINT,
    wallet,
    {
      registry: myRegistry,
      // gasPrice: {
